<template>
  <div>
    ...
    <a-modal width="1000px" v-model:visible="comModalVisible" title="组件显示">
      <com />
    </a-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async del(row) {
      ...
      let res = await configuredFormDelete(params);
      this.getTableData();
    ...
    async editHandleOk() {
      if (this.modalType == "edit") {
        ...
        let res = await configuredFormUpdate(params);
        this.getTableData();
        this.addModalVisible = false;
        res.data.code == "200"
          ? this.$message.success(res.data.message)
        ...
        if (this.addFrom) {
          ...
          console.log(params);
          let result = await configuredFormInsert(params);
          this.addFrom = {};
          this.getTableData();
          res.data.code == "200"
            ? this.$message.success(res.data.message)

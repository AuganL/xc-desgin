<template>
  <div>
    ...
    <a-modal width="1200px"  v-model:visible="comModalVisible" title="组件显示">
      <com />
    </a-modal>
  </div>
</template>
...
import { table, tableUpdate, tableDelete, tableInsert } from "../../../api/index";
export default {
  ...
  methods: {
    ...
    async del(row) {
      ...
      let res = await tableDelete(params);
      this.getTableData();
    ...
    async editHandleOk() {
      if (this.modalType == "edit") {
        ...
        if (this.addFrom) {
          ...
          console.log(params);
          let result = await tableInsert(params);
          this.addFrom = {};
          this.getTableData();

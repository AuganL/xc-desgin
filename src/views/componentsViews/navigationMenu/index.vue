...
export default {
  ...
  methods: {
    ...
    async del(row) {
      ...
      let res = await navigationMenuDelete(params);
      this.getTableData();
    ...
    async editHandleOk() {
      if (this.modalType == "edit") {
        ...
        let res = await navigationMenuUpdate(params);
        this.getTableData();
        this.addModalVisible = false;
        res.data.code == "200"
          ? this.$message.success(res.data.message)
        ...
        if (this.addFrom) {
          ...
          console.log(params);
          let result = await navigationMenuInsert(params);
          this.addFrom = {};
          this.getTableData();
